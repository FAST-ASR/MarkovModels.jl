...
function removenilstates(fsm::AbstractFSM{T}) where T
    ...
    while ! isempty(stack)
        ...
        for link in links(state)
            #if isemitting(link.dest) || islabeled(link.dest) || isinit(state) || isfinal(link.dest)
...
function compose(fsm::AbstractFSM{T}, subfsms::Dict) where T
    ...
    for state in states(fsm)
        ...
        if state.label ∈ keys(subfsms)
            ...
            newstates = Dict(initstate(s_fsm) => addstate!(nfsm, label = state.label),
                             finalstate(s_fsm) => addstate!(nfsm))
