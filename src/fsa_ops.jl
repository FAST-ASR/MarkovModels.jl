...
At every step, `fn(x) -> y` is called where `x` is a sparse vector
similar to `fsa.α` containing the weight associated to each active
state (inactive states have weight `zero{K}` where `K` is the FSA
semiring). The return value of `fn(x)` is filtered version of `x`
...
Trim `fsa` by removing states and arcs that are not on successful
paths.
"""
