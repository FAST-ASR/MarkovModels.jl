...
# Default iterator, no pruning.
Base.iterate(fsa::FSA) = (fsa.α, (fsa.T', fsa.α))
function Base.iterate(fsa::FSA, state)
    Tᵀ, vn_1 = state
...
# Acyclic iterator: each state is visited at most once.
struct AcyclicIterator{F}
    fsa::F
end
...
function Base.iterate(it::AcyclicIterator, state)
